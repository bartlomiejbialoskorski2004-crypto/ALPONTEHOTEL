...
export default function InformationsToc() {
  ...
  // Lock background scroll while the sheet is open (pause Lenis + body
  // overflow fallback), and allow Escape to dismiss.
  useEffect(() => {
    if (!sheetOpen) return;
    const lenis = getLenis();
    lenis?.stop();
  ...
  const handleSheetNav = (e: MouseEvent<HTMLAnchorElement>, id: string) => {
    ...
    setActiveId(id);
    setSheetOpen(false);
    // Let the sheet begin closing before the scroll kicks off.
